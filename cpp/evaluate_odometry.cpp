...
bool eval (string prediction_dir) {
  // ground truth and result directories
  string gt_dir         = "/home/jiange/dl/tensorflow-jiange/vo_gt/T-12d";
  ...
  for (int32_t j=0; j<sizeof(seq)/sizeof(seq[0]); j++) {
	int32_t i = seq[j];
    ...
    vector<Matrix> poses_result = loadPoses(result_dir + "/prediction/" + file_name);
   
    // plot status
    printf("Processing: %s, poses: %zu/%zu",file_name,poses_result.size(),poses_gt.size());
